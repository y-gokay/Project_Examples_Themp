...
const EditJobPost = () => {
  ...
  useEffect(() => {
    ...
    professionSearchTimeoutRef.current = setTimeout(async () => {
      const result = await searchProfessions(professionSearch, 100, 1);
      if (result.success) {
  ...
  useEffect(() => {
    if (jobPost) {
      // Set form data from job post
      setFormData({
        postTitle: jobPost.postTitle || "",
        postDescription: jobPost.postDescription || "",
        qualifications: jobPost.qualifications || "",
        professionId: jobPost.professionId?.toString() || "",
        workingMethodId: jobPost.workingMethodId?.toString() || "",
        hiringCount: jobPost.hiringCount?.toString() || "",
        minExperienceYear: jobPost.minExperienceYear?.toString() || "",
        ageMin: jobPost.ageMin?.toString() || "",
        ageMax: jobPost.ageMax?.toString() || "",
        weeklyWorkingHours: jobPost.weeklyWorkingHours?.toString() || "",
        genderOption:
          jobPost.genderOption !== undefined ? jobPost.genderOption : null,
        militaryStatus: jobPost.militaryStatus || null,
        ...
        applicationUntilDate: jobPost.applicationUntilDate
          ...
          : "",
        districts:
          jobPost.districts?.map(
            (d) => d.districtId?.toString() || d.district?.id?.toString(),
          ) || [],
        applicantRights:
          jobPost.applicantRights?.map(
            (ar) =>
              ar.applicantRightId?.toString() ||
              ar.applicantRight?.id?.toString(),
          ) || [],
        drivingLisenceRequirements:
          jobPost.drivingLisenceRequirements?.map(
            (dl) =>
              dl.drivingLisenceId?.toString() ||
              dl.drivingLisenceType?.id?.toString(),
          ) || [],
        educationLevels:
          jobPost.educationLevels?.map(
            (el) =>
              el.educationTypeId?.toString() ||
              el.educationType?.id?.toString(),
          ) || [],
        workDays:
          jobPost.workDays?.map(
            (wd) => wd.dayId?.toString() || wd.day?.id?.toString(),
          ) || [],
        workingExperiences:
          jobPost.workingExperiences?.map(
            (we) =>
              we.workingExperienceId?.toString() ||
              we.workingExperience?.id?.toString(),
          ) || [],
        languages:
          jobPost.languages?.map(
            (lang) =>
              lang.languageId?.toString() || lang.language?.id?.toString(),
          ) || [],
      });

      // Meslek adını set et
      if (jobPost.professionId) {
        const profession = lookups.professions?.find(
          (p) => p.id?.toString() === jobPost.professionId?.toString(),
        );
        if (profession) {
          setSelectedProfessionName(
            profession.profession || profession.title || profession.name || "",
          );
          setProfessionSearch(
            profession.profession || profession.title || profession.name || "",
          );
      ...
      // Set city if districts exist and load districts
      if (jobPost.districts && jobPost.districts.length > 0) {
        const firstDistrict = jobPost.districts[0];
        const cityId = firstDistrict.district?.cityId;
        if (cityId) {
          const cityIdStr = cityId.toString();
          setSelectedCityId(cityIdStr);
          // İlçeleri hemen yükle
          loadDistricts(cityIdStr);
        }
      }
    }
  }, [jobPost]); // eslint-disable-line react-hooks/exhaustive-deps

  // Meslek adını set et (lookups yüklendikten sonra)
  useEffect(() => {
    if (jobPost?.professionId && lookups.professions?.length > 0) {
      const profession = lookups.professions.find(
        (p) => p.id?.toString() === jobPost.professionId?.toString(),
      );
      if (profession && !selectedProfessionName) {
  ...
  }, [jobPost, lookups.professions, selectedProfessionName]);
  ...
  const loadJobPost = async () => {
    ...
    try {
      // Önce liste endpoint'inden tüm ilanları çek ve ID'ye göre bul
      const allJobs = await loadAllJobPosts();
      if (allJobs.length > 0) {
        const jobPost = allJobs.find(
          (job) => job.id?.toString() === id.toString(),
        );
        if (jobPost) {
          setJobPost(jobPost);
        } else {
          // Eğer listede bulunamazsa, direkt endpoint'i dene
          const result = await getBusinessJobPost(id);
          if (result.success && result.data) {
            setJobPost(result.data);
          } else {
            showToast({
              type: "error",
              message: result.error || "İlan bulunamadı",
              duration: 3000,
            });
            navigate(ROUTES.EMPLOYER_MY_JOBS);
          }
        ...
        // Liste çekilemezse direkt endpoint'i dene
        const result = await getBusinessJobPost(id);
        if (result.success && result.data) {
          setJobPost(result.data);
        } else {
          showToast({
            type: "error",
            message: result.error || "İlan yüklenemedi",
            duration: 3000,
          });
          navigate(ROUTES.EMPLOYER_MY_JOBS);
        }
      ...
      logError("Error loading job post:", err);
      showToast({
        type: "error",
        message: err.message || "Bağlantı hatası. Lütfen tekrar deneyin.",
        duration: 3000,
      });
      navigate(ROUTES.EMPLOYER_MY_JOBS);
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    await updateJobApi.execute(() => updateBusinessJobPost(id, submitData), {
      successMessage: "İlan başarıyla güncellendi",
      onSuccess: () => {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <Button
            variant="outline"
            onClick={() => navigate(ROUTES.EMPLOYER_MY_JOBS)}
            className="mb-4"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        // Eğer meslek seçiliyse ve kullanıcı yeni bir şey yazıyorsa, seçimi temizle
                        if (
                          formData.professionId &&
                    ...
                    {professionSearch && !formData.professionId && (
                      <div className="absolute z-10 mt-1 w-full max-h-60 overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 shadow-lg">
                        {searchingProfessions ? (
                          <div className="p-3 text-sm text-gray-500 dark:text-gray-400 text-center">
                            Aranıyor...
                          </div>
                        ) : professionOptions.length > 0 ? (
                          professionOptions.slice(0, 10).map((option) => (
                            <div
                              key={option.value}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 p-4 border-2 border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 min-h-[80px] max-h-[200px] overflow-y-auto">
                  ...
                  lookups.educationTypes.length > 0 ? (
                    // Eğitim seviyelerini düşükten yükseğe sırala
          ...
          <div className="flex gap-4">
            <Button
              type="submit"
              disabled={isSubmitting || loading || updateJobApi.loading}
