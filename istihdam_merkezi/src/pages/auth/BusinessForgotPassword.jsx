...
const BusinessForgotPassword = () => {
  const { forgotPasswordBusiness, loading, theme, toggleTheme } = useAppStore();
  ...
  const handleSubmitPhone = async (e) => {
    ...
    const result = await forgotPasswordBusiness(normalizedPhone);
    if (result.success) {
      showToast({
        type: "success",
  ...
  return (
    <div className="min-h-screen flex relative bg-gradient-to-br from-blue-50 to-white dark:from-gray-900 dark:to-gray-800">
      <SEOHead title="İşveren Şifre Sıfırlama" path="/isveren/sifremi-unuttum" noindex />
      ...
      {/* Sol tarafta görsel alan - işveren login ile uyumlu */}
...
const ResetPasswordStep = ({
  ...
  const { verifyResetOtpBusiness, resetPasswordBusiness, loading } =
    useAppStore();
  ...
  const handleVerifyOtp = async (e) => {
    ...
    const result = await verifyResetOtpBusiness(normalizedPhone, formData.otp);
    if (result.success) {
  ...
  const handleResetPassword = async (e) => {
    ...
    const result = await resetPasswordBusiness(
      resetToken,
